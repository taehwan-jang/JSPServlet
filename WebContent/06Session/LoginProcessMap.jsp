...
/*
	연습문제] 회원정보를 MemberDTO에 저장한 후 반횐했던 부분을
		Map컬렉션에 저장한 후 반환하여 처리할 수 있또록 수정하시오.
*/
if(map.get(id)!=null){
	//로그인에 성공한 경우 session영역에 회원인증정보를 저장한다.
	session.setAttribute("USER_ID", map.get(id));
	session.setAttribute("USER_PW", map.get(pw));
	...
	//로그인 페이지로 포워드(전달)
	request.getRequestDispatcher("Login.jsp").forward(request, response);//이거 잘 이해안가
}
