...
만약 값이 입력되지 않았다면 길이가 0인 String객체를 반환하고ㅅ
...
if(!(name==null || name.equals(""))){
	...
	//전성된 이름이 없다면 아래와 같이 JS를 브라우저에 출력한다.
...
//	out.println("</script>");
%>
<!-- 위와 같은 방법은 불편하므로 스크립트릿을 분리한 후 HTML며영역을
만들어 JS를 기술한다. 하지만 서브렛과정에서 코드가 지저분해진다. -->
<script>
	alert('이름을 입력하세요');
	history.back();
</script>
...
/*
checkbox는 2개 이상의 항목을 선택할 수 있으므로 getParameterValues()를
사용한다. 항목중 체크된것만 서버로 전송되고, 파라미터명이 틀리거나 값이 전송되지 않으면
null을 반환한다.
*/
...
*/
String self_intro = request.getParameter(
			"self_intro").replace("\r\n","<br />");
%>
...
<%
/*
getParameterNames() : 폼값의 이름ㅇ르 한꺼번에 받아올때 사용하는
...
while(names.hasMoreElements()){
	...
	if(paramName.equals("sex")||paramName.equals("favorite")){
		...
		System.out.println("getParameterValues()로" 
					+" 얻어서 값을 출력함<br/>");
	}
	else{
