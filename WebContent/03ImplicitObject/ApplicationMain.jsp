...
<body>
	...
	<ul>
		...
		<li>this키워드(주로 선언부에서 사용): <br />
			<%=this.getServletContext().getRealPath("/images") %></li>
	</ul>
	
	<h2>선언부에서 물리적경로 사용하기</h2>
	<%-- 
		application내장객체는 ServelContext타입을 가지고 있다.
	 --%>
	<%!
	//1.this를 사용해서 ServletContext객체 얻어오기
	String getRealPath(){
		return 
	...
	*/
	...
	%>
	<ul>
		<li>this키워드로 사용 : <br /> <%=getRealPath() %></li>
		<li>매개변수로 전달 : <br /> <%=getRealPath(application) %></li>
		<%
		this.app2 = application;
		%>
		<li>멤버변수 : <br /> <%=getRealPathString() %></li>
	</ul>
</body>
</html>